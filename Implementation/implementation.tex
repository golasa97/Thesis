...
This was converted into the \textit{true} average total cross section $\langle \sigma_t \rangle$ by use of the correction factor calculated from SESH, which was then fit with SAMMY.

Once new URR parameters were obtained, they were used to calculate a new correction factor. This was then used to obtain a new corrected cross section, and a new fit was performed, producing new resonance parameters. This process was repeated until the resonance parameters converged onto a final value, which was typically achieved within approximately three iterations.

The implementation described in the following sections integrates the self-shielding correction capabilities of SESH directly into SAMMY's fitting engine. This creates a new, streamlined workflow where the evaluator provides the actual measurement data (such as average transmission $\langle T \rangle$ or average capture yield $\langle Y \rangle$) directly to the code. The average capture yield is the energy-averaged ratio of detected capture events to incident neutrons, and is subject to self-shielding effects analogous to those in transmission. The treatment of average capture yield is performed in \autoref{chap:capture-yield-correction}. SAMMY now internally calculates the necessary self-shielding correction factors during each iteration and applies them to the theoretical calculation, rather than requiring the evaluator to correct the experimental data externally. The new and improved workflow is shown in \autoref{fig:current-workflow}.
\begin{figure}[h]
    \centering
...
While $D_0$ does not appear explicitly in the average total cross section (\autoref{eq:hauser-feshbach-cross-section}), it enters the formalism through the non-elastic partial cross sections. The energy-averaged partial cross section for an incident channel $a$ and exit channel $b$ can be written as a product of the Hauser-Feshbach compound nucleus term and a width fluctuation correction\cite{Moldauer1980}:
\begin{equation}
    \label{eq:partial-cross-section}
    \langle \sigma_{ab} \rangle = \frac{\pi g_a}{k_a^2}\, \frac{T_a T_b}{T} \int_0^\infty dt\; e^{-i T_\gamma t / T} \prod_{c \notin \gamma} \left( 1 + \frac{2\, T_c}{\nu_c\, T}\, t \right)^{-\nu_c/2 - \delta_{ac} - \delta_{bc}}
\end{equation}
where the quantities to the left of the integral sign constitute the Hauser-Feshbach expression, and the integral is the Moldauer prescription\cite{Moldauer1976} for the width fluctuation correction factor. Here $a$ represents the incident channel and $b$ the exit channel; $\nu_c$ and $T_c$ represent the number of degrees of freedom (multiplicity) and the transmission coefficient, respectively, for channel $c$. Subscript $\gamma$ refers to photon channels, and $T = \sum_c T_c$ is defined as the sum over all channels.

For neutron channels, $T_c$ is determined by the strength function $S_\ell$ and the distant-level parameter $R_\ell^\infty$, the same quantities that appear in the average total cross section. For photon and fission channels, however, the channel probabilities for a given spin $J$ are\cite{Moldauer1980}:
\begin{equation}
    \label{eq:photon-transmission}
    T_\gamma = 2\pi\, \frac{\langle \Gamma_\gamma \rangle}{D_J} \qquad \text{and} \qquad T_f = 2\pi\, \frac{\langle \Gamma_f \rangle}{D_J}
\end{equation}
where $D_J$ is the mean level spacing for levels with spin $J$, obtained from $D_0$ via a nuclear level density model as described in \autoref{ssec:level-density}. It is through these channel probabilities that $D_0$ enters the calculation of partial cross sections.

The parameter $D_0$ is not varied during the SAMMY fitting process; it is determined from analysis of the resolved resonance region and is provided as a fixed input by the evaluator. Consequently, the parameters that SAMMY adjusts during fitting are the strength functions $S_\ell$, the distant-level parameters $R_\ell^\infty$, and the average radiative width $\langle \Gamma_\gamma \rangle$.
...
\label{sec:param-conversion}
    ...
    \subsection{Calculating Available Channels}
        ...
        \begin{equation}
            ...
            \end{array}
        \end{equation}
        The physical significance of the multiplicity is that it determines the number of degrees of freedom for the $\chi^2$ distribution from which neutron widths are sampled\cite{Porter1956}. When $\mu = 2$, the neutron width is sampled from a $\chi^2$ distribution with two degrees of freedom rather than one. By utilizing this assumption, the number of resonance ladders that must be independently sampled is nearly halved, since two channel spins sharing the same $J$ are treated as a single effective channel.
    ...
    \subsection{Calculating $J$-Dependent Level Spacings}
        ...
        \end{equation}
        in which $A$ is the mass number of the target nucleus, $a_{\text{ld}}$ is the level density parameter (in MeV$^{-1}$), $B_E$ is the binding energy (in MeV), and $P_E$ is the pairing energy (in MeV). These quantities are obtained from the Gilbert-Cameron composite level density model\cite{gc-leveldensity}. The Bethe formula allows $D_J/D_0$ to evolve as a function of energy more realistically compared to the simple approximation in \autoref{eq:simple-j-dependency}, particularly for high-$J$ states and at higher energies where the spin cutoff parameter varies significantly.
        ...
        The validation of this model was done by comparing the new SESH-calculated level spacings with those from the \textsuperscript{181}Ta evaluation in ENDF-8.1\cite{endf-8.1}. The ENDF-8.1 evaluation stores $J$-dependent level spacings that were computed by SAMMY using its implementation of the Bethe formula during the evaluation process. These level spacings are tabulated in the ENDF File~2 resonance parameter data. The purpose of this comparison is therefore to verify that SESH's new implementation of the Bethe formula (\autoref{eq:bethe-formula}) produces $J$-dependent level spacings consistent with SAMMY's implementation, ensuring that the two codes are using the same underlying physics model.
        ...
        As shown in \autoref{fig:level=spacing=validation}, the level spacings calculated by the updated SESH implementation now agree with the SAMMY-produced values stored in the ENDF-8.1 evaluation across all spin groups and energies. While the simpler model in \autoref{eq:simple-j-dependency} approximates the level spacing adequately for s-wave channels, the two formulas diverge at higher $\ell$ values where the spin cutoff parameter plays a larger role. Implementing the Bethe formula (\autoref{eq:bethe-formula}) resolves these discrepancies and ensures consistency between SESH and SAMMY.
    ...
    \label{ssec:neutron-widths}
        ...
        \label{eq:penetration-factor}
            ...
            \end{cases}
        \end{equation}
        For $\ell > 2$, the penetrability, shift factor, and phase shift are computed using the standard recursive relations\cite{Blatt1952}\cite{Lane1958}:
        \begin{align}
            P_\ell &= \frac{\rho^2\, P_{\ell-1}}{(\ell - \mathcal{S}_{\ell-1})^2 + P_{\ell-1}^2} \label{eq:pen-recursion} \\[6pt]
            \mathcal{S}_\ell &= \frac{\rho^2\, (\ell - \mathcal{S}_{\ell-1})}{(\ell - \mathcal{S}_{\ell-1})^2 + P_{\ell-1}^2} - \ell \label{eq:shift-recursion}
        \end{align}
        where $P_\ell$ is the penetrability and $\mathcal{S}_\ell$ is the shift factor (not to be confused with the strength function $S_\ell$), with the initial values $P_0 = \rho$ and $\mathcal{S}_0 = 0$. For $\ell \leq 2$, these recursions reduce to the explicit forms in \autoref{eq:penetration-factor}. In practice, evaluations in ENDF-8.1 rarely include more than three partial waves ($\ell \leq 2$), but the recursion is implemented to handle arbitrary $\ell$ values for generality.
    ...
    \begin{equation}
        ...
        \end{cases}
    \end{equation}
        For $\ell > 2$, the phase shift is computed using the same recursion used for the penetrability and shift factors (\autoref{eq:pen-recursion}--\autoref{eq:shift-recursion}):
        \begin{equation}
            \label{eq:phase-recursion}
            \phi_\ell = \phi_{\ell-1} - \arctan{\frac{P_{\ell-1}}{\ell - \mathcal{S}_{\ell-1}}}
        \end{equation}
        with $\phi_0 = \hat{\rho}$, where $P_{\ell-1}$ and $\mathcal{S}_{\ell-1}$ are evaluated using $\hat{\rho}$ in place of $\rho$. As noted in \autoref{ssec:neutron-widths}, the explicit forms given above are sufficient for the vast majority of current ENDF evaluations.
        ...
        The generalized SPRT method\cite{sprt} provides an improved model that extends beyond the s-wave low-energy approximation. It gives an approximate relationship for the effective scattering radius $R'_\ell$ that accounts for higher-order partial waves:
        \begin{equation}
            \label{eq:sprt}
...
This procedure rests on two assumptions about the energy bin over which $\langle \sigma \rangle$ is defined. First, the bin must be wide enough to contain a statistically meaningful number of resonances, so that the sampled ensemble accurately represents the distribution of cross-section values within the bin. Second, the bin must be narrow enough that all slowly varying energy-dependent quantities (e.g., the potential scattering cross section, the penetrability factors, and the average resonance parameters themselves) can be treated as constant across the bin. Under these conditions, each sampled realization uses the same values of $\sigma_p$, $V_\ell$, $D_J$, and $\langle \Gamma_{n,J}^\ell \rangle$, and only the stochastically sampled resonance parameters ($E_r$, $\Gamma_{n,r}$) vary between realizations. These are the defining conditions of the idealized URR: average parameters are purely statistical, and the self-shielding correction depends only on the variance they produce. The consequences of violating these assumptions (particularly the finite-resonance effect that arises when the bin contains too few resonances) are partially addressed in \autoref{chap:uncertainty}.

The second assumption merits further justification. In principle, one could sample resonance energies first and then evaluate the energy-dependent average parameters ($D_J(E)$, $\langle \Gamma_{n,J}^\ell \rangle(E)$, etc.)\ at each sampled resonance energy. However, as described in \autoref{sec:weighted-wigner-validation}, SESH does not build a full resonance ladder across the entire URR; instead, it samples only a small number of resonance pairs in the immediate vicinity of the energy of interest using the weighted Wigner distribution. The verification in \autoref{chap:transmission-correction} demonstrates that approximately four resonance pairs per channel are sufficient to reproduce the full-ladder correction factor. This means the sampled resonances span a range of roughly $4 D_J$ in energy around the point of interest. Over such a narrow interval, the average resonance parameters vary negligibly. For example, in the $J=3$, $\ell=0$ channel of \textsuperscript{181}Ta\cite{endf-8.1}, the level spacing is approximately 9~eV, so four level spacings span roughly 36~eV. Over this range the level spacing changes by less than 0.1\% at any energy in the URR, and the reduced neutron width varies by a comparable amount. Evaluating the energy dependence of the average parameters at each sampled resonance energy would therefore require repeated calls to the level density model and penetrability calculations for a correction that is negligible compared to the dominant source of cross-section variance, namely the Porter-Thomas fluctuations in neutron width.
...
\end{equation}
where $P(s)$ is the Wigner distribution defined in \autoref{eq:wigner-distribution}. The numerator weights each spacing by its size $s$, and the denominator normalizes the distribution. This weighting ensures that the probability of sampling a given spacing is proportional to both its Wigner probability and its width, correctly accounting for the fact that a uniformly distributed neutron energy is more likely to fall within a wider spacing.
