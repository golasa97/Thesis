This section describes the implementation of the physics model and cross-section sampling into SAMMY as required for simulating resonances and approximating self-shielded theoretical measurements from URR parameters.
...
\end{equation}
This derived value, which as previously stated is not the true cross section, but the self-shielded cross section.  This is dependent on experimental conditions (sample thickness, sample temperature, sample composition, etc.), and needed to be corrected in order to be fit with SAMMY.
%\begin{figure}
%    \centering
...
The average cross section in the URR is typically defined according to Hauser-Feshbach theory\cite{Hauser1952}, which provides a framework for describing nuclear reactions proceeding through a compound nucleus when many channels are open and individual resonance effects are smeared out. The energy-averaged total cross section for an incoming channel $c$ can be expressed as:
...
\section{Converting from SAMMY Parameters to SESH Parameters}
    A critical step in the integration involves reconciling the parameter sets used by the two codes. SAMMY operates primarily on R-Matrix parameters---specifically the strength function ($S_\ell$) and the distant level parameter ($R_\ell^\infty$) defined for each orbital angular momentum $\ell$. However, the SESH self-shielding calculation requires a statistical ladder of individual resonances. To bridge this gap, the average R-Matrix parameters must be converted into statistical resonance parameters: the J-dependent average level spacing ($D_J$) and the average neutron width ($\langle \Gamma_n \rangle$). This conversion enables the stochastic generation of resonance structures that are statistically consistent with the average R-Matrix parameters.
    ...
    \subsection{Improving Level Density J Dependency}
        ...
        The updated methodology uses the same input S-wave level spacing, but instead uses Bethe formula\cite{Bethe1936} to describe the conversion from $D_0$ to $D_J$
        \begin{equation}
            \label{eq:bethe-formula}
            \frac{1}{D_J} = \frac{1}{D_0}
            \left\{ \exp{ \left[ \frac{-J^2}{2\sigma(E)^2}\right]} - \exp{ \left[ \frac{-(J+1)^2}{2\sigma(E)^2}\right]}
            \right\} 
        \end{equation}
        ...
        It should also be noted that \textsuperscript{181}Ta has a relatively well-behaved URR, with statistical properties that are well-described by the Hauser-Feshbach theory. The disagreement between the two formulas would be much larger for nuclei with less-than-ideal URRs (e.g., \textsuperscript{90}Zr), which exhibit non-statistical effects such as intermediate structure or strong level-level interference.

    \subsection{Converting Strength Functions to Neutron Widths}
        With the complete set of available channels and the $J$ and $E$ dependent level spacings calculated in the previous sections, the per-channel average neutron width, $\langle \Gamma_{n,J}^{\ell} \rangle$, can be calculated. This quantity is calculated for each unique $(\ell,J)$ by:
        \begin{equation}
            \label{eq:avg-neut-width}
            \langle \Gamma_{n,J}^{\ell} \rangle = \mu_{\ell,J} D_J S_\ell \nu_{\ell}(E) \sqrt{E}
        \end{equation}
        in which $S_ell$ is the strength function (a parameter that is either input or fit to experimental data), and $\nu_{\ell}$ is the penetrability\cite{Blatt1952}, which has an $\ell$ and $\rho$ dependent form given in \autoref{eq:penetration-factor}.
        \begin{equation}
    ...
    \subsection{Calculating Potential Scattering}
        ...
        The final component required for the cross-section simulation is the potential scattering\cite{Teichmann1952}. As shown in \autoref{eq:slbw}, the first part of that equation exists outside the resonance sum. This is the potential scattering term\cite{t2}, and can be written as
        \begin{equation}
...
\autoref{eq:phase-shift}.
    ...
    \end{equation}
        ...
        \end{equation}
        However, for analyses that extend to higher energies or include higher orbital momenta $(\ell > 0)$, this s-wave approximation is considered inadequate\cite{jeff18}\cite{Lynn1968}. A more sophisticated model is required to accurately replicate the underlying cross-section behavior and its associated resonance structure.
