...
The consequence of this self-shielding uncertainty having experimental dependencies beyond just the resonance statistics introduces complexity into the binning structure when it comes to performing an evaluation.

A thin sample can have a more certain self-shielding correction factor
Depending on the isotope, it may not be feasible to get good statistics from a thin, minimally self-shielded sample
Conversely, a thicker sample would have a different requirement for how wide a bin has to be to optimize the self-shielding correction assumptions.
That bin could end up being prohibitively wide for both thin and thick sample transmissions.
Instead, a better method would be to remove that statistical constraint and incorporate that uncertainty into the evaluation.
