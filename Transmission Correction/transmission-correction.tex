% --- Added for clarity (advisor comments): scope and reference model ---
This chapter validates the transmission correction workflow as implemented for use within SAMMY.
The correction factor $C_T$ is computed using the modernized SESH transport-based approach and is exercised inside SAMMY's fitting pipeline.
Validation is performed by comparison to an MCNP6 reference transport model constructed for the same geometries and using the same evaluated nuclear data, so the comparison primarily tests the correction method and its numerical implementation rather than the underlying cross-section evaluation.
...
Before integrating the transmission correction into the fitting procedure, it is essential to verify that SESH accurately calculates the correction factor itself. The verification was performed by comparing the results from SESH to those from a high-fidelity model of the experiment constructed using MCNP \cite{mcnp}. This MCNP model serves as a computational benchmark\cite{Humbert2014}, providing a ground truth against which the statistical methods in SESH can be compared. Three key physical parameters that could influence the correction factor were investigated: the number of external resonance pairs used in the calculation, the physical thickness of the sample, and the temperature of the sample.

\subsection{MCNP6 reference Model}
...
The results, shown in \autoref{fig:tc-resonance-pairs}, demonstrate that the inclusion of external resonance pairs has a slight but noticeable influence on the calculated correction factor. As more pairs are added, the SESH calculation converges toward the MCNP benchmark value. \autoref{fig:tc-resonance-pairs-error} shows the relative error between SESH and MCNP, confirming that using more resonance pairs produces more accurate results. For the remainder of this work, four external resonance pairs were used in all SESH calculations to ensure a balance between accuracy and computational efficiency.
 ...
 \begin{figure}[H]
     ...
     \caption{Comparison of the transmission correction factor for a 4~mm $^{181}$Ta sample at 300~K, calculated with SESH using a varying number of external resonance pairs and compared to the MCNP benchmark.}
 ...
 \begin{figure}[H]
     ...
     \caption{Relative error between SESH and MCNP for varying the total number of external resonances.}
...
As shown in \autoref{fig:tc-thickness} and \autoref{fig:tc-thickness-error}, there is very strong agreement between SESH and MCNP for the thinner samples. However, as the sample thickness increases, SESH begins to under-predict the correction factor. This deviation becomes more pronounced for thicker samples, particularly where the correction factor $C_T$ exceeds a value of 1.5 For practical planning, this provides a simple rule-of-thumb: thicknesses that keep $C_T$ below this threshold in the validated energy range are expected to behave well in the fitting workflow.. This analysis suggests that to maintain a disagreement of less than 1\%, the use of this method should be limited to samples where the calculated correction factor remains below this threshold.
 ...
 \begin{figure}[H]
     ...
     \caption{Relative error between SESH and MCNP for simulations of various thicknesses.}
